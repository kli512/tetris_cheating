...
for shape_name, shape in _occupied_shapes.items():
    occupied_spots = [[] for rot in range(4)]
    for rot in range(4):
        for r_i, row in enumerate(shape[rot]):
            for c_i, val in enumerate(row):
                if val != 0:
                    occupied_spots[rot].append((r_i, c_i))

    OCCUPIED[shape_name] = np.array(occupied_spots)

# WIP this isn't even close to being finished
POS_OCCUPIED = {}

POS_OCCUPIED['T'] = None

POS_OCCUPIED = {
    'T': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
    'L': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
    'J': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
    'I': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
    'O': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
    'S': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
    'Z': [np.argwhere(piece_grid != 0) for piece_grid in OCCUPIED['T']],
}
